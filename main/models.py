...
class Waybill(MongoModel):
    ...
    def to_json(self):
        return {
            ...
            'delivery_time': self.delivery_time.strftime("%Y/%m/%d %H:%M") if self.delivery_time is not None else "",
            'etd': self.etd.strftime("%Y/%m/%d %H:%M") if self.etd is not None else "",
            'eta': self.eta.strftime("%Y/%m/%d %H:%M") if self.eta is not None else "",
