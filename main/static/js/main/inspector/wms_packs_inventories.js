    ...
    table = $("#table_details")
        .on('error.dt', function (e, settings, techNote, message) {
            ...
            ajax: function (data, callback, settings) {
                ...
                $.ajax({
                    type: "GET",
                    url: "/api/v1.0/users/wms_packs_inventories",
                    cache: false, //禁用缓存
                    data: param, //传入组装的参数
                    dataType: "json",
            ...
            "columns": [
                ...
                {'data': 'inbound_num',  "orderable": false
                },
                {'data': null,  "orderable": false,
                    render: function (data, type, full) {
                        html_str = "<table class='table no-footer'><tbody>"
                        for(var i=0;i<data.products.length;i++){
                            html_str = html_str + "<tr><td>"+data.products[i].name_cn+"</td><td>"+data.products[i].name_en+"</td></tr>"
                        }
                        html_str = html_str + "</tbody></table>"
                        return html_str
                    }
                },
                {'data': 'available_quantity',  "orderable": false
                },
            ],
